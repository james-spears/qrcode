 ...
 */
function getSegments(regex: RegExp, mode: Mode.Mode, str: string): Segment[] {
    const segments = [];
    let result;
...
function mergeSegments(segs: Segment[]): Segment[] {
    return segs.reduce(function (acc: Segment[], curr) {
...
function buildGraph(
    ...
    table: Record<string, { node: Segment; lastCount: number }>;
} {
 ...
 */
function buildSingleSegment(data: string, modesHint: Mode.Mode | string | null): Segment {
    let mode;
    const bestMode = Mode.getBestModeForData(data);

    if (modesHint) mode = Mode.from(modesHint, bestMode);

    // Make sure data can be encoded
    if (mode && mode !== Mode.BYTE && mode.bit < bestMode.bit) {
        throw new Error(
            '"' +
                data +
                '"' +
 ...
 */
export const fromArray = (array: (string | Segment)[]): Segment[] => {
    return array.reduce(function (acc: Segment[], seg) {
 ...
 * @return {Array}       Array of segments
 */
export const rawSplit = (data: string): Segment[] => {
